...
- Use the **test API key** for sandbox requests and the **live API key** for production requests.  
...
To prevent duplicate transactions caused by network failures or retries, the API supports **idempotency**, ensuring that repeated requests execute only once.  

- Include an `Idempotency-Key: <key>` header in your request.  
- If a request with the same key has already been processed, the API will return a response with the header: `X-Idempotent-Replay: true`.  
